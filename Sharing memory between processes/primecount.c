...
int main (int argc, char **argv)
{
  ...
  totalCount=mmap(NULL,sizeof(int)*1,PROT_WRITE|PROT_READ,MAP_SHARED | MAP_ANONYMOUS,-1,0);//sharing total  counter between processes
  if(flagarr==MAP_FAILED || totalCount==MAP_FAILED)
  {
    exit(1);
  }
  /*****************************************************************************************
  ...
  ************************************************/
  ...
  if (pid>0)
  {
    while(wait(NULL)>0);//waiting to all child processes to exit
    count=*totalCount;// summing their child processes counter of prime numbers
    //Print results
